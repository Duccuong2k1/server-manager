...
export default function CustomerMapWrapper() {
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] sm:p-6">
      <div className="mb-2">
        <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">Server Distribution</h3>
        <p className="mt-1 text-gray-500 text-theme-sm dark:text-gray-400">Số lượng server theo quốc gia</p>
      </div>
              ...
              stats.countryStats.slice(0, 5).map((stat) => (
                <div key={stat.country} className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div>
                      <p className="font-semibold text-gray-800 text-theme-sm dark:text-white/90">{stat.country}</p>
                      <span className="block text-gray-500 text-theme-xs dark:text-gray-400">{stat.count} Servers</span>
                    </div>
                  </div>
                  ...
                  <div className="flex w-full max-w-[140px] items-center gap-3">
                    ...
                    <p className="font-medium text-gray-800 text-theme-sm dark:text-white/90">{stat.percentage}%</p>
                  </div>
                </div>
              ))
            ) : (
